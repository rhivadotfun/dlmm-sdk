...
use crate::{
    ...
    conversions::{CollectFeeMode, FunctionType, TokenProgramFlagWrapper},
    types::{ActivationType, PairStatus, PairType, TokenProgramFlags},
};
...
impl LbPairExtension for LbPair {
    ...
    fn is_support_limit_order(&self) -> bool {
        let Some(function_type) = FunctionType::try_from(self.parameters.function_type).ok() else {
            return false;
        };
        match function_type {
